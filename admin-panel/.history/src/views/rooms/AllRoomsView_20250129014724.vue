...
<template>
  ...
  <section
    class="w-9/12 mx-auto px-10 h-full grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-10"
  >
    <div v-for="room in state.rooms" :key="room.roomNumber" :room="room">
      <div
        ...
        class="rounded-lg shadow-lg py-6 h-[15rem] flex flex-col items-center justify-between"
      >
        <h3 class="text-xl font-semibold text-center mb-2">
          Room {{ room.roomNumber }}
        </h3>
        <div class="flex items-center justify-center mb-2">
          <FontAwesomeIcon class="text-gray-600 text-xl" :icon="faUsers" />
          <p class="ml-2">
            Capacity <strong>{{ room.capacity }} </strong> residents
          </p>
        </div>
        <div class="flex items-center justify-center mb-2">
          <FontAwesomeIcon class="text-gray-600 text-xl" :icon="faStairs" />
          <p class="ml-2">Floor {{ room.floor }}</p>
        </div>
        <div
          v-if="room.hasSeaView"
          class="flex items-center justify-center mb-2"
        >
          <FontAwesomeIcon class="text-gray-600 text-xl" :icon="faWater" />
          <p class="ml-2">Sea View</p>
        </div>
        <div v-else class="flex items-center justify-center mb-2">
          <FontAwesomeIcon class="text-gray-600 text-xl" :icon="faMountain" />
          <p class="ml-2">Mountain View</p>
        </div>
      </div>
    </div>
  </section>
      ...
      <ul class="flex items-center -space-x-px h-10 text-base">
        ...
        <li v-for="page in state.pagesNumber" :key="page">
          <a
            ...
            :class="[
              pageIndex === page - 1
                ? 'bg-gray-700 text-black hover:bg-gray-700 hover:text-white'
                : 'bg-white ',
            ]"
